...
ℓ ∈ Sig =
  foldr (λ ℓ′ → if Σ.proj₁ (ℓ ≟ ℓ′) then (λ _ → ⊤) else id) ⊥ (labels Sig)
...
Restrict : ∀ {s} (Sig : Signature s) (ℓ : Label) → ℓ ∈ Sig →
           Signature s
Restrict ∅              ℓ ()
Restrict (Sig , ℓ′ ∶ A) ℓ ℓ∈ with Σ.proj₁ (ℓ ≟ ℓ′)
... | true  = Sig
...
Proj ∅              ℓ {}
Proj (Sig , ℓ′ ∶ A) ℓ {ℓ∈} with Σ.proj₁ (ℓ ≟ ℓ′)
... | true  = A
... | false = Proj Sig ℓ {ℓ∈}
Proj (_,_≔_ Sig ℓ′ {A = A} a) ℓ {ℓ∈} with Σ.proj₁ (ℓ ≟ ℓ′)
... | true  = A
... | false = Proj Sig ℓ {ℓ∈}
...
_∣_ {Sig = ∅}            r       ℓ {}
_∣_ {Sig = Sig , ℓ′ ∶ A} (rec r) ℓ {ℓ∈} with Σ.proj₁ (ℓ ≟ ℓ′)
... | true  = Σ.proj₁ r
...
mutual
  ...
  _With_≔_ ∅ ℓ {} a
  _With_≔_ (Sig , ℓ′ ∶ A)   ℓ {ℓ∈} a with Σ.proj₁ (ℓ ≟ ℓ′)
  ... | true  = Sig                   , ℓ′ ≔ a
  ...
  drop-With {Sig = ∅} {ℓ∈ = ()}      r
  drop-With {Sig = Sig , ℓ′ ∶ A} {ℓ} (rec r) with Σ.proj₁ (ℓ ≟ ℓ′)
